...
const serverRequest = (route, errorText, method = 'GET', body = null) =>
  ...
  fetch(`${BASE_URL}${route}`, {method, body})
    // Возвращаем объект promise с будущим ответом сервера и, если он придёт успешнр, вызываем метод then и передаём ему колбэк с объектом ответа response
    .then((response) => {
    ...
    // В случае прихода ошибки с сервера (перехода в состояние  rejected) вызываем метод catch и передаём ему колбэк с текстом ошибки
    .catch(() => {
